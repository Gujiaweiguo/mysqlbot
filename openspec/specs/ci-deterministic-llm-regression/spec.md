# ci-deterministic-llm-regression Specification

## Purpose
Provide a deterministic CI-only LLM provider path for G4 happy-path regression so GitHub Actions can validate NL-to-SQL behavior without depending on external model credentials or output variability.
## Requirements
