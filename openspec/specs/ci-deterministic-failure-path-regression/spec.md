# ci-deterministic-failure-path-regression Specification

## Purpose
Provide a deterministic CI failure-path provider contract for G5 so GitHub Actions can validate rate-limit and transient-provider behavior without depending on uncontrolled external services.
