...
高级小助手（高级应用）在“配置接口”步骤中，除了接口 URL、超时和接口凭证外，还支持工作空间与数据源范围配置。下面两项最容易混淆：

### 1. AES Key / AES IV 从哪里来

- 系统**不会自动生成** `AES Key` / `AES IV`。
- 这两项是 **mySQLBot 与第三方接口之间约定好的共享密钥**，需要由接入双方统一生成并分别配置到两边。
- 只有在第三方接口返回的数据源字段已经经过 AES 加密时，才需要开启“AES 加密”。

当前实现使用：

- **AES-256-CBC**
- **PKCS7 / PKCS5Padding**
- `AES Key` 取前 **32** 个字符（不足会补齐）
- `AES IV` 取前 **16** 个字符（不足会补齐）
...
- **接口凭证当前是必填项**，至少需要添加 **1 条** 才能保存高级小助手。
- 一条接口凭证包含以下字段：

| 字段 | 含义 | 常见填写方式 |
|------|------|-------------|
| 凭证名称 | 这条凭证的备注名 | `qa-token` |
| 源系统凭证类型 | 凭证来源说明 | 一般填 `custom` |
| 目标凭证名称 | 发给第三方接口的字段名 | `Authorization` / `X-API-Key` / `access_token` |
| 目标凭证位置 | 凭证放在请求的哪里 | `header` / `cookie` / `param` |
| 目标凭证 | 凭证值本身 | `Bearer your-token` |

如果第三方接口使用请求头鉴权，推荐这样配置：

- 凭证名称：`api-token`
- 源系统凭证类型：`custom`
- 目标凭证名称：`Authorization`
- 目标凭证位置：`header`
- 目标凭证：`Bearer your-token`
...
如果第三方接口要求把 token 放到 URL 参数或 Cookie 中，只需要把“目标凭证位置”改成 `param` 或 `cookie`，并把“目标凭证名称”改成对方接口要求的字段名即可。
