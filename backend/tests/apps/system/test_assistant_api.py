...
class FakeSession:
    def __init__(
        ...
        self.workspaces: dict[int, WorkspaceModel] = workspaces or {}
        self.datasources: dict[int, CoreDatasource] = datasources or {}
...
class FakeLLMService:
    def __init__(self, assistant: AssistantHeader) -> None:
        self.current_assistant: AssistantHeader | None = assistant
        self.out_ds_instance: AssistantOutDs | None = None
...
def test_normalize_configuration_persists_multi_workspace_selection(
    ...
    assert config["default_datasource_id"] is None
...
def test_normalize_configuration_persists_default_datasource_when_auto_enabled(
    admin_user: UserInfoDTO,
) -> None:
    session = FakeSession(
        workspaces={1: _build_workspace(1, "Default")},
        datasources={11: _build_datasource(11, 1, "Orders")},
    )
    ...
    assert normalized is not None
    config = json.loads(normalized)
    assert config["auto_ds"] is True
    assert config["default_datasource_id"] == 11


def test_normalize_configuration_rejects_missing_default_datasource_when_auto_enabled(
    admin_user: UserInfoDTO,
) -> None:
    session = FakeSession(
        workspaces={1: _build_workspace(1, "Default")},
    ...
    with pytest.raises(Exception, match="requires a default datasource"):
        _normalize_and_validate_configuration(
            session=cast(Session, cast(object, session)),
            current_user=admin_user,
            assistant_type=0,
...
def test_normalize_configuration_rejects_default_datasource_outside_scope(
    admin_user: UserInfoDTO,
) -> None:
    session = FakeSession(
        workspaces={1: _build_workspace(1, "Default")},
        datasources={
            11: _build_datasource(11, 1, "Orders"),
            22: _build_datasource(22, 1, "CRM"),
        },
    )

    with pytest.raises(
        Exception, match="must be within the assistant datasource scope"
    ):
        _normalize_and_validate_configuration(
            session=cast(Session, cast(object, session)),
            current_user=admin_user,
