...
def generate_password_reset_token(email: str) -> str:
    expire_hours = getattr(settings, "EMAIL_RESET_TOKEN_EXPIRE_HOURS", 1)
    delta = timedelta(hours=int(expire_hours))
...
def deepcopy_ignore_extra(src: Any, dest: Any) -> Any:
    import copy
...
def setup_logging() -> None:
    ...
    # 日志格式
    formatter = logging.Formatter(f"{settings.LOG_FORMAT}")
...
class SQLBotLogUtil:
    ...
    def _get_logger() -> logging.Logger:
        ...
        try:
            if frame is None or frame.f_back is None or frame.f_back.f_back is None:
                return CallerLogger(logging.getLogger("__main__"))
    ...
    @staticmethod
    def error(
        msg: str, *args: Any, exc_info: bool | None = None, **kwargs: Any
    ) -> None:
        logger = SQLBotLogUtil._get_logger()
    ...
    def critical(msg: str, *args: Any, **kwargs: Any) -> None:
        ...
        if logger.isEnabledFor(logging.CRITICAL):
            logger.log(logging.CRITICAL, msg, *args, **kwargs)


def prepare_for_orjson(data: Any) -> Any:
    if not data:
        return data
...
def prepare_model_arg(origin_arg: Any) -> Any:
    if not isinstance(origin_arg, str):
        return origin_arg
    if not origin_arg.strip() or origin_arg.strip()[0] not in {"{", "["}:
        return origin_arg
    try:
...
def get_origin_from_referer(request: Request) -> str | None:
...
def equals_ignore_case(str1: str | None, *args: str | None) -> bool:
    if str1 is None:
        return None in args
    for arg in args:
        if arg is None:
