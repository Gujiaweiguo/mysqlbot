...
class SecurityConfig(BaseModel):
    ...
    ssl_cert_path: str | None = Field(
        default=None, description="Path to custom CA bundle for SSL verification"
    )

    # JWT Settings
    jwt_verify_signature: bool = Field(
        default=True, description="Enable JWT signature verification"
    )

    jwt_verify_expiration: bool = Field(
        default=True, description="Enable JWT expiration verification"
    )

    # Request Timeout Settings
    default_request_timeout: int = Field(
        default=30, description="Default timeout for HTTP requests in seconds"
    )

    database_connection_timeout: int = Field(
        default=10, description="Default timeout for database connections in seconds"
    )
    ...
    require_password_uppercase: bool = Field(
        default=True, description="Require at least one uppercase letter in passwords"
    )

    require_password_lowercase: bool = Field(
        default=True, description="Require at least one lowercase letter in passwords"
    )

    require_password_digit: bool = Field(
        default=True, description="Require at least one digit in passwords"
    )

    require_password_special: bool = Field(
        default=True, description="Require at least one special character in passwords"
    )
    ...
    rate_limit_per_minute: int = Field(
        default=60, description="Maximum requests per minute per user"
    )
    ...
    sanitize_html_input: bool = Field(
        default=True, description="Sanitize HTML input to prevent XSS attacks"
    )

    # CSRF Protection
    enable_csrf_protection: bool = Field(
        default=True, description="Enable CSRF protection for state-changing requests"
    )

    # Logging and Monitoring
    log_security_events: bool = Field(
        default=True, description="Log security-related events"
    )
    ...
    account_lockout_duration_minutes: int = Field(
        default=15, description="Duration of account lockout in minutes"
    )
...
def validate_password_strength(
    password: str, config: SecurityConfig = DEFAULT_SECURITY_CONFIG
) -> tuple[bool, str]:
    """
