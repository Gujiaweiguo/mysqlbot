...
@router.get(
    "/page/{current_page}/{page_size}", summary=f"{PLACEHOLDER_PREFIX}get_term_page"
)
...
async def pager(
    ...
    dslist: list[int] | None = Query(None, description="数据集ID集合(可选)"),
) -> dict[str, Any]:
    current_page, page_size, total_count, total_pages, _list = page_terminology(
        session, current_page, page_size, word, current_user.oid, dslist
    )
...
@require_permissions(
    permission=SqlbotPermission(
        role=["ws_admin"], type="ds", keyExpression="info.datasource_ids"
    )
)
@system_log(
    LogConfig(
...
@require_permissions(permission=SqlbotPermission(role=["ws_admin"]))
async def delete(session: SessionDep, id_list: list[int]) -> None:
...
@require_permissions(permission=SqlbotPermission(role=["ws_admin"]))
async def enable(session: SessionDep, id: int, enabled: bool, trans: Trans) -> None:
    enable_terminology(session, id, enabled, trans)


@router.get("/export", summary=f"{PLACEHOLDER_PREFIX}export_term")
@system_log(
    LogConfig(operation_type=OperationType.EXPORT, module=OperationModules.TERMINOLOGY)
)
    ...
    def inner() -> io.BytesIO:
        ...
        for obj in _list:
            _data = {
                ...
                "datasource": ", ".join(obj.datasource_names)
                if obj.datasource_names and obj.specific_ds
                else "",
            }
        ...
        fields.append(
            AxisObj(name=trans("i18n_terminology.synonyms"), value="other_words")
        )
        fields.append(
            AxisObj(
                name=trans("i18n_terminology.term_description"), value="description"
            )
        )
        fields.append(
            AxisObj(
                name=trans("i18n_terminology.effective_data_sources"),
                value="datasource",
            )
        )
        fields.append(
            AxisObj(
                name=trans("i18n_terminology.all_data_sources"),
...
async def excel_template(trans: Trans) -> StreamingResponse:
    def inner() -> io.BytesIO:
        ...
        _data1 = {
            ...
            "description": trans(
                "i18n_terminology.term_description_template_example_1"
            ),
            "all_data_sources": "N",
            "datasource": trans(
                "i18n_terminology.effective_data_sources_template_example_1"
            ),
        }
        ...
        _data2 = {
            ...
            "description": trans(
                "i18n_terminology.term_description_template_example_2"
            ),
            "all_data_sources": "Y",
            "datasource": "",
        }
        ...
        fields = []
        fields.append(
            AxisObj(name=trans("i18n_terminology.term_name_template"), value="word")
        )
        fields.append(
            AxisObj(
                name=trans("i18n_terminology.synonyms_template"), value="other_words"
            )
        )
        fields.append(
            AxisObj(
                name=trans("i18n_terminology.term_description_template"),
                value="description",
            )
        )
        fields.append(
            AxisObj(
                name=trans("i18n_terminology.effective_data_sources_template"),
                value="datasource",
            )
        )
        fields.append(
            AxisObj(
                name=trans("i18n_terminology.all_data_sources_template"),
                value="all_data_sources",
            )
        )
...
@system_log(
    LogConfig(operation_type=OperationType.IMPORT, module=OperationModules.TERMINOLOGY)
)
async def upload_excel(
    trans: Trans, current_user: CurrentUser, file: UploadFile = File(...)
) -> dict[str, Any]:
    ALLOWED_EXTENSIONS = {"xlsx", "xls"}
    filename_value = file.filename or ""
    if not filename_value.lower().endswith(tuple(ALLOWED_EXTENSIONS)):
        raise HTTPException(400, "Only support .xlsx/.xls")

    os.makedirs(path, exist_ok=True)
    filename_parts = filename_value.split(".")
    base_name = filename_parts[0] if filename_parts else "terminology"
    ext_name = filename_parts[-1] if len(filename_parts) > 1 else "xlsx"
    base_filename = f"{base_name}_{hashlib.sha256(uuid.uuid4().bytes).hexdigest()[:10]}"
    ...
    def inner() -> dict[str, Any]:
        ...
        for sheet_name in sheet_names:
            if get_excel_column_count(save_path, str(sheet_name)) < len(use_cols):
            ...
            for _index, row in df.iterrows():
                ...
                other_words = (
                    [w.strip() for w in row[1].strip().split(",")]
                    if pd.notna(row[1]) and row[1].strip()
                    else []
                )
                description = (
                    row[2].strip() if pd.notna(row[2]) and row[2].strip() else ""
                )
                datasource_names = (
                    [d.strip() for d in row[3].strip().split(",")]
                    if pd.notna(row[3]) and row[3].strip()
                    else []
                )
                all_datasource = (
                    True
                    if pd.notna(row[4])
                    and row[4].lower().strip() in ["y", "yes", "true"]
                    else False
                )
        ...
        failed_records = cast(list[dict[str, Any]], res["failed_records"])
        ...
        if len(failed_records) > 0:
            ...
            for obj in failed_records:
                _data = {
                    "word": obj["data"].word,
                    "other_words": ", ".join(obj["data"].other_words)
                    if obj["data"].other_words
                    else "",
                    "description": obj["data"].description,
                    "all_data_sources": "N" if obj["data"].specific_ds else "Y",
                    "datasource": ", ".join(obj["data"].datasource_names)
                    if obj["data"].datasource_names and obj["data"].specific_ds
                    else "",
                    "errors": obj["errors"],
                }
                data_list.append(_data)
            ...
            fields = []
            fields.append(
                AxisObj(name=trans("i18n_terminology.term_name"), value="word")
            )
            fields.append(
                AxisObj(name=trans("i18n_terminology.synonyms"), value="other_words")
            )
