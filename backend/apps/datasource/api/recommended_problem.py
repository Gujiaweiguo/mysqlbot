...
from apps.datasource.crud.recommended_problem import (
    get_datasource_recommended,
    get_datasource_recommended_base,
...
async def datasource_recommended(session: SessionDep, ds_id: int) -> Any:
...
async def datasource_recommended_base(session: SessionDep, ds_id: int) -> Any:
...
async def save_datasource_recommended(
    session: SessionDep, user: CurrentUser, data_info: RecommendedProblemBase
) -> Any:
    if data_info.datasource_id is None or data_info.recommended_config is None:
        raise Exception("datasource_id and recommended_config are required")
    update_ds_recommended_config(
        session, data_info.datasource_id, data_info.recommended_config
    )
