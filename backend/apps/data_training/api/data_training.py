...
class FailedTrainingRecord(TypedDict):
    data: DataTrainingInfo
    errors: list[str]


@router.get(
    "/page/{current_page}/{page_size}", summary=f"{PLACEHOLDER_PREFIX}get_dt_page"
)
...
async def pager(
    ...
    current_page, page_size, total_count, total_pages, _list = page_data_training(
        session, current_page, page_size, question, current_user.oid
    )
...
@require_permissions(
    permission=SqlbotPermission(
        role=["ws_admin"], type="ds", keyExpression="info.datasource"
    )
)
@system_log(
    LogConfig(
...
@system_log(
    LogConfig(
        operation_type=OperationType.EXPORT, module=OperationModules.DATA_TRAINING
    )
)
async def export_excel(
    ...
    trans: Trans,
    current_user: CurrentUser,
    question: str | None = Query(None, description="搜索术语(可选)"),
) -> StreamingResponse:
    def inner() -> io.BytesIO:
        ...
        fields.append(
            AxisObj(
                name=trans("i18n_data_training.problem_description"), value="question"
            )
        )
        fields.append(
            AxisObj(name=trans("i18n_data_training.sample_sql"), value="description")
        )
        fields.append(
            AxisObj(
                name=trans("i18n_data_training.effective_data_sources"),
        ...
        if current_user.oid == 1:
            fields.append(
                AxisObj(
                    name=trans("i18n_data_training.advanced_application"),
...
async def excel_template(trans: Trans, current_user: CurrentUser) -> StreamingResponse:
    def inner() -> io.BytesIO:
        data_list = []
        _data1 = {
        ...
        fields.append(
            AxisObj(
                name=trans("i18n_data_training.problem_description_template"),
                value="question",
            )
        )
        fields.append(
            AxisObj(
                name=trans("i18n_data_training.sample_sql_template"),
                value="description",
            )
        )
        fields.append(
            AxisObj(
                name=trans("i18n_data_training.effective_data_sources_template"),
                value="datasource_name",
            )
        )
        if current_user.oid == 1:
            fields.append(
                AxisObj(
                    name=trans("i18n_data_training.advanced_application_template"),
                    value="advanced_application_name",
                )
            )
...
@system_log(
    LogConfig(
        operation_type=OperationType.IMPORT, module=OperationModules.DATA_TRAINING
    )
)
async def upload_excel(
    ...
    base_filename = f"{file_name.split('.')[0]}_{hashlib.sha256(uuid.uuid4().bytes).hexdigest()[:10]}"
    filename = f"{base_filename}.{file_name.split('.')[1]}"
    ...
    def inner() -> dict[str, Any]:
        ...
        for sheet_name in sheet_names:
            ...
            if get_excel_column_count(save_path, sheet_name_str) < len(use_cols):
            ...
            for _, row in df.iterrows():
                ...
                description = (
                    row[1].strip() if pd.notna(row[1]) and row[1].strip() else ""
                )
                datasource_name = (
                ...
                if oid == 1 and len(row) > 3:
                    advanced_application_name = (
                        row[3].strip() if pd.notna(row[3]) and row[3].strip() else ""
                    )
        ...
        failed_records = cast(
            list[FailedTrainingRecord],
            res.get("failed_records", []),
        )
        ...
        if len(failed_records) > 0:
            ...
            fields.append(
                AxisObj(
                    name=trans("i18n_data_training.problem_description"),
                    value="question",
                )
            )
            fields.append(
                AxisObj(
                    name=trans("i18n_data_training.sample_sql"), value="description"
                )
            )
            fields.append(
                AxisObj(
