...
from apps.system.crud.user_excel import (
    UploadResultDTO,
    batchUpload,
    download_error_file,
    downTemplate,
...
async def templateExcel(trans: Trans) -> StreamingResponse:
...
@require_permissions(permission=SqlbotPermission(role=["admin"]))
async def download_error(file_id: str) -> FileResponse:
...
async def pager(
    ...
    oidlist: list[int] | None = Query(None, description=f"{PLACEHOLDER_PREFIX}oid"),
    ...
    if oidlist:
        origin_stmt = origin_stmt.where(col(UserWsModel.oid).in_(oidlist))
    if origins:
        origin_stmt = origin_stmt.where(col(UserModel.origin).in_(origins))
    if status is not None:
        origin_stmt = origin_stmt.where(col(UserModel.status) == status)
    if keyword:
    ...
    user_page = await paginator.get_paginated_response(
        stmt=origin_stmt, pagination=pagination, **filters
    )
    ...
    stmt: Any = (
        ...
        .join(UserWsModel, col(UserModel.id) == col(UserWsModel.uid), isouter=True)
        .where(col(UserModel.id).in_(uid_list))
...
def format_user_dict(row: Row[tuple[Any, ...]]) -> dict[str, Any]:
    result_dict: dict[str, Any] = {}
    for item, key in zip(row, row._fields, strict=True):
...
@clear_cache(
    ...
    cacheName=str(CacheName.USER_INFO),
    keyExpression="current_user.id",
)
@system_log(
    LogConfig(
...
async def ws_change(
    ...
    user_model = get_db_user(session=session, user_id=current_user.id)
...
async def query(
    ...
    db_user = get_db_user(session=session, user_id=id)
...
async def create(session: SessionDep, creator: UserCreator, trans: Trans) -> UserModel:
    if check_account_exists(session=session, account=creator.account):
        raise Exception(
            trans("i18n_exist", msg=f"{trans('i18n_user.account')} [{creator.account}]")
        )
    if check_email_exists(session=session, email=creator.email):
        raise Exception(
            trans("i18n_exist", msg=f"{trans('i18n_user.email')} [{creator.email}]")
        )
    if not check_email_format(creator.email):
        raise Exception(
            trans(
                "i18n_format_invalid",
                key=f"{trans('i18n_user.email')} [{creator.email}]",
            )
        )
    ...
    if creator.oid_list:
        ...
        db_model_list = [
            UserWsModel.model_validate({"oid": oid, "uid": user_model.id, "weight": 0})
...
async def update(session: SessionDep, editor: UserEditor, trans: Trans) -> None:
    user_model = get_db_user(session=session, user_id=editor.id)
    if not user_model:
    ...
    if editor.email != user_model.email and check_email_exists(
        session=session, email=editor.email
    ):
        raise Exception(
            trans("i18n_exist", msg=f"{trans('i18n_user.email')} [{editor.email}]")
        )
    if not check_email_format(editor.email):
        raise Exception(
            trans(
                "i18n_format_invalid",
                key=f"{trans('i18n_user.email')} [{editor.email}]",
            )
        )
    origin_oid: int = user_model.oid

    uws_list_stmt = select(UserWsModel).where(col(UserWsModel.uid) == editor.id)
    ...
    if oids_to_remove:
        del_stmt = sqlmodel_delete(UserWsModel).where(
            col(UserWsModel.uid) == editor.id, col(UserWsModel.oid).in_(oids_to_remove)
        )
    ...
    if editor.oid_list:
        user_model.oid = (
            origin_oid if origin_oid in editor.oid_list else editor.oid_list[0]
        )
        if oids_to_add:
            db_uws_model_list = [
                UserWsModel.model_validate(
                    {"oid": oid, "uid": user_model.id, "weight": 0}
                )
...
async def batch_del(session: SessionDep, id_list: list[int]) -> None:
    for id in id_list:
        await single_delete(session, id)
...
async def langChange(
    session: SessionDep, current_user: CurrentUser, trans: Trans, language: UserLanguage
) -> None:
...
async def statusChange(
    ...
    db_user = get_db_user(session=session, user_id=statusDto.id)
    assert db_user is not None
